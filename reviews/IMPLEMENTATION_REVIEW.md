...
## Ciclo 23 — Teodora como agente operacional de estudo de massa

### Problema e prioridade

O protótipo possuía análises, ações locais e comparação, mas essas capacidades apareciam como recursos separados. Alertas terminavam em texto; alternativas vinham de snapshots fixos; o painel da torre era predominantemente informativo; e o usuário não via uma cadeia explícita entre intenção, geração, aplicação e decisão.

### Princípios constitucionais afetados

- cada interação deve mover o projeto;
- inteligência é a interface;
- o chat comanda e explica, enquanto a decisão acontece no comparador;
- alternativas devem representar estratégias e trade-offs;
- mudanças materiais precisam de preview, undo e histórico;
- recomendações devem declarar objetivo, risco, confiança e próximo passo.

### Solução implementada

- motor puro `generateMassingAlternatives()` gera três estratégias a partir do estado atual:
  - Menor risco regulatório;
  - Máximo de unidades seguro;
  - Mais área livre;
- cada alternativa contém geometria, métricas calculadas, risco, impacto e recomendação por objetivo;
- comandos em linguagem natural reconhecem densidade, risco, redistribuição, área livre, explicação e aplicação da recomendada;
- a mensagem de alternativas mostra o fluxo Intenção → Diagnóstico → Gerar → Comparar → Recomendar → Aplicar → Registrar;
- cards apresentam unidades, CA, TO, risco, impacto e indicação recomendada;
- “Aplique a alternativa recomendada” produz prévia calculada e ação executável;
- alertas do canvas ganharam CTA “Gerar alternativas”;
- o resumo do lote oferece “Gerar 3 alternativas seguras”;
- o painel da torre ganhou otimização com Teodora, redistribuição, variação e explicação da contribuição no CA;
- o comparador ganhou área livre, risco regulatório, “Ver no canvas” e “Aplicar esta versão”;
- histórico “Decisões do estudo” registra geração, inspeção, aplicação, mudança de premissas e decisão, incluindo deltas de CA e unidades;
- análises genéricas do painel foram substituídas por contribuição no CA, concentração de altura, trade-off e recomendação concreta.
...
- novo teste confirma três alternativas distintas, calculadas, aplicáveis e recomendação coerente com o objetivo;
- cenário inicial produz:
  - Menor risco: 157 unidades, CA 10,4, risco baixo;
  - Máximo seguro: 179 unidades, CA 11,9, risco médio;
  - Mais área livre: 145 unidades, CA 9,4, TO 7,3%, risco baixo;
- `git diff --check`: aprovado;
- JSX compilado e JavaScript parseado com Babel Standalone 8.0.2: 211.313 bytes sem erro;
- busca confirmou CTAs operacionais em alertas, painel, cards e comparador.

### Limitações e riscos

- o motor é determinístico e simulado; não otimiza circulação, insolação, estrutura, vagas ou geometria regulatória completa;
- risco considera principalmente consumo de CA e validade dos cálculos;
- histórico e alternativas permanecem somente em memória;
- alternativas geradas não recebem identidade persistente de cenário;
- a inspeção visual automatizada não foi possível porque o navegador integrado estava indisponível;
- legislação continua informada/simulada e não validada por fonte oficial.

### Oportunidades descobertas

- persistir `AgentRun`, `MassingAlternative`, `Recommendation` e `DecisionEvent`;
- adicionar preview temporário de alternativa sem aplicá-la;
- ligar cada evento do histórico ao objeto afetado no canvas;
- incorporar vagas, permeabilidade, gabarito e complexidade ao avaliador;
- permitir travar torres durante geração.

### Recomendação para o próximo ciclo

Validar visualmente o fluxo completo em navegador real e, após aprovação da V0, iniciar a migração Vite + TypeScript promovendo as novas entidades operacionais para módulos persistíveis.
